          ...
          <div style={{display:'flex', alignItems:'flex-start', justifyContent:'space-between', gap:'12px 16px', marginBottom:'16px', flexWrap:'wrap'}}>
            ...
            <div className="dv-pqc-bubble dv-badge" onClick={() => setShowPqcTip(!showPqcTip)} style={{position:'relative', display:'inline-flex', alignItems:'center', gap:'6px', fontSize:'10px', padding:'6px 10px', borderRadius:'2px', background: fileMetadata?.pqc ? 'rgba(197,52,0,0.08)' : 'rgba(239,68,68,0.08)', border: '1px solid var(--hairline)', color: fileMetadata?.pqc ? 'var(--accent)' : '#ef4444', cursor:'pointer', flexShrink:0, alignSelf:'flex-start', letterSpacing:'0.14em', fontFamily:'var(--font-mono)', textTransform:'uppercase', backdropFilter:'blur(12px)'}}>
              ...
              <div className="dv-pqc-tooltip" style={{position:'absolute', top:'calc(100% + 10px)', right:0, width:'300px', maxWidth:'calc(100vw - 32px)', background:'var(--raised)', backdropFilter:'blur(20px)', border:'1px solid var(--hairline)', borderRadius:'2px', padding:'14px 16px', fontSize:'13px', lineHeight:'1.6', color:'var(--text-secondary)', boxShadow:'0 12px 32px rgba(0,0,0,0.5)', opacity: showPqcTip ? 1 : 0, pointerEvents: showPqcTip ? 'auto' : 'none', transition:'all 0.2s', zIndex:10, textAlign:'left', fontFamily:'var(--font-body)', textTransform:'none', letterSpacing:'0'}}>
                {fileMetadata?.pqc ? 'This vault was created over a TLS 1.3 connection confirmed by the edge — quantum-safe against harvest-now-decrypt-later attacks.' : 'This vault was created without PQC protection (HNDL risk). Files uploaded after the PQC update are marked PQC secure.'}
              </div>
            </div>
          </div>
