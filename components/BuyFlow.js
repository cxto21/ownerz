...
export default function BuyFlow({
  ...
  const handlePurchase = async () => {
    ...
    try {
      ...
      if (!locked) {
        throw new Error(
          'No vault found for this CID. The file may not have been uploaded with FileVault.'
        );
      }
      ...
      const isClaimed =
        locked.isClaimed ??
        locked.is_claimed ??
        locked.lock?.is_claimed ??
        false;
      if (status !== 0 || isClaimed) {
        // Show lock details for debugging
        const commitment =
          locked.commitment ?? locked.lock?.commitment ?? 'unknown';
        throw new Error(
      ...
      const sellerAddress =
        locked.issuer ?? locked.seller ?? locked.vault?.seller;
      ...
      const platformFeeBps =
        locked.platformFeeBps ??
        locked.meta?.platformFeeBps ??
        locked.vault?.platform_fee_bps ??
        100;
  ...
  const handleStrk20Payment = async () => {
    ...
    try {
      ...
      if (platformWallet) {
        ...
        console.warn(
          '[BuyFlow] NEXT_PUBLIC_PLATFORM_WALLET not set — skipping platform fee, seller receives full price'
        );
      }
  ...
  const handleClaim = async () => {
    ...
    try {
      ...
      if (
        msg.includes('INVALID_PROOF') ||
        msg.includes('0x494e56414c49445f50524f4f46')
      ) {
        msg =
          'Invalid claim secret — INVALID_PROOF. Check the secret from the seller and try again.';
    ...
    <>
      ...
      {step > 0 && step < 8 && (
        <div className="dv-progress">
          ...
          <div
            className={`dv-progress-step ${step >= 3 ? 'done' : step === 2 ? 'active' : ''}`}
          ></div>
          <div
            className={`dv-progress-step ${step >= 4 ? 'done' : step === 3 ? 'active' : ''}`}
          ></div>
          <div
            className={`dv-progress-step ${step >= 6 ? 'done' : step >= 4 ? 'active' : ''}`}
          ></div>
      ...
      {step === 0 && (
        <>
          <div>
            ...
            <p className="dv-hint">
              Get the CID from the seller. Pay with STRK to receive the
              decryption key.
            </p>
          </div>
      ...
      {step === 2 && (
        <>
          ...
          >
            <div
              className="dv-pqc-bubble"
              onClick={() => setShowPqcTip(!showPqcTip)}
              style={{
                position: 'relative',
                display: 'inline-flex',
                alignItems: 'center',
                ...
                padding: '4px 8px',
                borderRadius: '999px',
                background: fileMetadata?.pqc
                  ? 'rgba(112,145,255,0.10)'
                  : 'rgba(239,68,68,0.12)',
                border: fileMetadata?.pqc
                  ? '1px solid rgba(91,112,168,0.30)'
                  : '1px solid rgba(239,68,68,0.25)',
                color: fileMetadata?.pqc ? 'rgba(153,176,255,.9)' : '#ef4444',
                cursor: 'pointer',
              }}
            >
              <span
              ...
              <div
                ...
                style={{
                  ...
                  opacity: showPqcTip ? 1 : 0,
                  pointerEvents: showPqcTip ? 'auto' : 'none',
              ...
              >
                {fileMetadata?.pqc
                  ? 'Your browser is using TLS 1.3 with end-to-end PQC (Post-Quantum Cryptography) active — your connection is quantum-safe.'
          ...
          {fileMetadata && (
            <div className="dv-metadata-card">
              <div className="dv-metadata-row">
                ...
                <span className="dv-metadata-value">
                  {cid ? cid.slice(0, 20) + '...' : ''}
                </span>
              </div>
              <div className="dv-metadata-row">
                <span className="dv-metadata-label">Price</span>
                <span className="dv-metadata-value price">
                  {fileMetadata.price} STRK
                </span>
              </div>
              <div className="dv-metadata-row">
                <span className="dv-metadata-label">Platform fee 1%</span>
                <span className="dv-metadata-value fee">
                  {(parseFloat(fileMetadata.price || 0) * 0.01).toFixed(4)} STRK
                </span>
              </div>
              <div className="dv-metadata-row">
              ...
              <div className="dv-metadata-row">
                ...
                <span
                  className="dv-metadata-value"
                  style={{ color: 'rgba(255,255,255,0.7)' }}
                >
                  {(parseFloat(fileMetadata.price || 0) * 0.99).toFixed(4)} STRK
                  (99%)
                </span>
              </div>
              {fileMetadata.commitment && (
                <>
                  <div className="dv-metadata-row">
                    ...
                    >
                      {fileMetadata.isClaimed ? 'claimed' : 'active'} ·{' '}
                      {String(fileMetadata.commitment).slice(0, 12)}...
                    </span>
                  </div>
              ...
              <div className="dv-metadata-row">
                ...
                <span
                  ...
                  style={{
                    fontSize: '13px',
                    fontWeight: 600,
                    color: fileMetadata.pqc ? '#22c55e' : '#f59e0b',
                  }}
                >
                  {fileMetadata.pqc
                    ? '✓ Created over PQC'
                    : '⚠ Created without PQC — HNDL risk'}
                </span>
              </div>
            </div>
          )}
          ...
          >
            You need at least {parseFloat(fileMetadata?.price || 0) + 6} STRK in
            your public balance (to shield and pay). Then pay privately from the
            pool. Seller receives 99%, platform 1%.
          </p>

          {error && <div className="dv-error">{error}</div>}

          <button className="dv-btn-primary" onClick={handleStrk20Payment}>
      ...
      {step === 4 && (
        <>
          <div>
            ...
            <p className="dv-hint">
              Enter the claim secret from the seller to recover the decryption
              key.
            </p>
          </div>
          ...
          {txHash && (
            <div className="dv-cid-box">
              <div className="dv-cid-header">
                ...
                <button
                  className="dv-copy"
                  onClick={() => copyToClipboard(txHash, 'tx', setCopied)}
                >
      ...
      {step === 8 && decryptedFile && (
        <>
          <div>
            ...
            <p className="dv-hint">
              File decrypted in your browser. Download it now.
            </p>
          </div>
          ...
          <button className="dv-btn-secondary" onClick={reset}>
            Purchase Another
          </button>
        </>
      )}
