...
export default function SellFlow({
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (!cid || cid === 'pending')
        throw new Error('Upload failed to return CID');
      ...
      if (lockResult?.transaction_hash) {
        try {
          ...
          if (!verifyLock) {
            throw new Error(
              'Transaction may have failed on-chain. Please try again.'
            );
          }
        }
        ...
        if (!verifyLock)
          console.warn(
            'Vault not yet visible on-chain, may need a few seconds'
          );
      }
    ...
    <>
      ...
      {step > 0 && step < 4 && (
        <div className="dv-progress">
          ...
          <div
            className={`dv-progress-step ${step >= 2 ? 'done' : step === 1 ? 'active' : ''}`}
          ></div>
          <div
        ...
        <>
          ...
          >
            <div>
              ...
              <p className="dv-hint" style={{ margin: '4px 0 0 0' }}>
                Encrypted and uploaded to F1 One (Filecoin). Any file type
                works.
              </p>
            </div>
            <div
              ...
              style={{
                ...
                padding: '5px 10px',
                borderRadius: '999px',
                background: 'rgba(112,145,255,0.10)',
                border: '1px solid rgba(91,112,168,0.30)',
            ...
            >
              ...
              >
                Update to a modern browser with TLS 1.3 to enable end-to-end PQC
                (Post-Quantum Cryptography) for your connection
              </div>
            </div>
          </div>
          ...
          {feeInfo && (
            <div className="dv-fee-display">
              ...
              <div className="dv-fee-breakdown">
                Base: {feeInfo.baseFee} STRK + Storage:{' '}
                {feeInfo.storageFeeFormatted} STRK
              </div>
            </div>
          )}
          ...
          <div className="dv-input-group">
            ...
            {price && !isNaN(parseFloat(price)) && parseFloat(price) > 0 && (
              ...
              >
                Platform fee 1% — you will receive 99% (
                {(parseFloat(price) * 0.99).toFixed(4)} STRK). No gas included.
                <div
                  style={{
                    fontSize: '13px',
                ...
                >
                  Fee: {(parseFloat(price) * 0.01).toFixed(4)} STRK · Seller
                  receives 99% at purchase time
                </div>
              </div>
            )}
          </div>
      ...
      {step === 2 && (
        <div className="dv-loading">
          ...
          <small style={{ color: 'rgba(255,255,255,0.4)', marginTop: '8px' }}>
            Approve the transaction in your wallet (fee + vault creation in one
            tx).
          </small>
        </div>
      )}
      ...
      {step === 3 && result && (
        <>
          ...
          >
            <div
              ...
              style={{
                ...
                padding: '5px 10px',
                borderRadius: '999px',
                background: result.pqc
                  ? 'rgba(112,145,255,0.10)'
                  : 'rgba(239,68,68,0.12)',
                border: result.pqc
                  ? '1px solid rgba(91,112,168,0.30)'
                  : '1px solid rgba(239,68,68,0.25)',
                color: result.pqc ? 'rgba(153,176,255,.9)' : '#ef4444',
                cursor: 'pointer',
              }}
            >
              <span
                style={{
                  ...
                  height: '5px',
                  borderRadius: '50%',
                  background: result.pqc ? 'rgba(153,176,255,.9)' : '#ef4444',
                  display: 'inline-block',
                }}
              ></span>
              ...
              >
                {result.pqc
                  ? 'Your browser is using TLS 1.3 with end-to-end PQC (Post-Quantum Cryptography) active — your vault was created quantum-safe.'
          ...
          <div>
            ...
            <p className="dv-hint">
              Share this CID and claim secret privately with your buyer.
            </p>
          </div>
          ...
          <div className="dv-cid-box">
            <div className="dv-cid-header">
              ...
              <button
                className="dv-copy"
                onClick={() => copyToClipboard(result.cid, 'cid', setCopied)}
              >
          ...
          <div className="dv-cid-box">
            <div className="dv-cid-header">
              ...
              <button
                className="dv-copy"
                onClick={() =>
                  copyToClipboard(result.claimSecret, 'secret', setCopied)
                }
              >
            ...
            >
              The raw secret key is stored encrypted in the vault. Only the
              claim secret can recover it.
            </small>
          </div>
          ...
          <div className="dv-info-row">
            ...
            >
              {result.pqc ? '✓ Created with PQC' : '⚠ Created without PQC'}
            </strong>
          </div>
