...
export default function DeploySection({ account, refreshWallet }) {
  ...
  if (deployed) {
    return (
      ...
      >
        <h3
          style={{ color: '#10b981', margin: '0 0 10px 0', fontSize: '14px' }}
        >
          ✅ Contracts Deployed
        </h3>
        {kexAddress && (
        ...
        >
          Add both to .env and restart dev server. See scripts/deploy.js for
          Sepolia steps.
        </div>
      </div>
    );
  ...
  return (
    ...
    >
      ...
      >
        For Sepolia:{' '}
        <code>node scripts/deploy.js --private-key 0x... --account 0x...</code>
        <br />
        This deploys KEX first, then FileVault with KEX address. See
        scripts/deploy.js comments.
      </p>
