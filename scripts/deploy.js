...
function loadArtifact(rel) {
  ...
  if (!fs.existsSync(p))
    throw new Error(`Artifact not found: ${p} — run 'scarb build' first`);
  return JSON.parse(fs.readFileSync(p, 'utf8'));
}

// FileVault (ownerz_filevault)
const fvSierraPath =
  'contracts/target/dev/ownerz_filevault_FileVault.contract_class.json';
const fvCasmPath =
  'contracts/target/dev/ownerz_filevault_FileVault.compiled_contract_class.json';
...
try {
  ...
  const artifacts = loadArtifact(
    'contracts/target/dev/ownerz_filevault.starknet_artifacts.json'
  );
  // not critical — starknet.js can declare without CASM on some RPCs, but Sepolia requires it
  console.warn(
    '[deploy] FileVault CASM not found at',
    fvCasmPath,
    '- declare may require CASM'
  );
}

// KeyExchangeMockup (key_onchain)
const kexSierraPath =
  'lib/key-onchain/target/dev/keyexchangemockup_KeyExchangeMockup.contract_class.json';
const kexCasmPath =
  'lib/key-onchain/target/dev/keyexchangemockup_KeyExchangeMockup.compiled_contract_class.json';
...
async function main() {
  ...
  try {
    ...
    console.warn(
      '[account] getNonce failed (account may not be deployed):',
  ...
  try {
    ...
    const declKex = await account.declare({
      contract: kexSierra,
      casm: kexCasm,
    });
  ...
  try {
    ...
    if (
      ...
      console.log(
        '[FV] already declared, but constructor changed - forcing re-declare by using new class hash'
      );
