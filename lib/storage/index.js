...
const S3_BASE =
  process.env.NEXT_PUBLIC_S3_BASE || 'https://eu-west-1.s3.fil.one/ownerz-v01';
...
export async function downloadKeySeed(cid) {
  ...
  const response = await fetch(
    `/api/download-key?key=${encodeURIComponent(key)}`
  );
...
export async function uploadEncryptedFile(cid, encryptedData, fileName) {
  ...
  return {
    key,
    cid: key,
    url: result.url || `${S3_BASE}/${key}`,
    etag: result.etag,
    pqc: result.pqc ?? false,
