...
// STRK20 pool contract address (from network config)
...
export async function privateTransfer(
  ...
  try {
    console.log('Private transfer attempt:', {
      tokenAddress,
      amount,
      recipientAddress,
    });
  ...
  try {
    ...
    if (result.pending) {
      return {
        ...
        transactionHash: null,
        message:
          'Batch transfer submitted to wallet. Check explorer for status.',
      };
    }
  ...
  try {
    ...
    if (balances && balances.length > 0) {
      ...
      return {
        ...
        message: bal
          ? `Shielded: ${fromSmallestUnit(bal.balance)} STRK`
          : 'No shielded balance',
      };
    }
    return {
