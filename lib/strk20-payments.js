...
export async function registerWallet(account) {
  try {
    ...
    console.log('Register wallet actions:', JSON.stringify(actions, (k, v) => typeof v === 'bigint' ? v.toString() : v))
...
export async function shieldTokens(account, tokenAddress, amount) {
  try {
    ...
    console.log('Shield actions:', JSON.stringify(actions, (k, v) => typeof v === 'bigint' ? v.toString() : v))
...
export async function privateTransfer(account, tokenAddress, amount, recipientAddress) {
  try {
    ...
    console.log('Transfer actions:', JSON.stringify(actions, (k, v) => typeof v === 'bigint' ? v.toString() : v))
    ...
    const userMsg = error.message?.includes('USER_REFUSED')
...
export async function batchPrivateTransfer(account, tokenAddress, transfers) {
  try {
    ...
    console.log('Batch transfer actions:', JSON.stringify(actions, (k, v) => typeof v === 'bigint' ? v.toString() : v))
...
export async function unshieldTokens(account, tokenAddress, amount, recipientAddress) {
  try {
    ...
    console.log('Unshield actions:', JSON.stringify(actions, (k, v) => typeof v === 'bigint' ? v.toString() : v))
    const result = await strk20InvokeViaWalletApi(session.account, actions)
