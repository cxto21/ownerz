...
export async function connectWallet(wallet) {
  try {
    ...
    try {
      ...
      // Fallback: requestAccounts + build a wallet-compatible signer
      // Use get-starknet's walletV6.requestAccounts to get the address
      try {
        ...
        // Absolute fallback: try to read address from wallet object itself
        address = wallet?.address || wallet?.selectedAddress || null
      }
      ...
      // Build a minimal account that can invoke via the wallet
      account = {
        address,
        wallet,
        invoke: async (calls) => {
          ...
          // Older wallets: use execute via starknet.js
    ...
    // Detect STRK20 — safe fallback to false on any error
    let isStrk20 = false
    try {
      isStrk20 = await isStrk20Capable(wallet)
    } catch { /* wallet may not support API version detection */ }
