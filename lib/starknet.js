...
export async function connectWallet(wallet) {
  try {
    ...
    try {
      ...
      const execFn = async (calls, ...rest) => {
        // Primary: use starknet.js Account (works with all wallets)
        try {
          const { Account } = await import('starknet')
          const tmpAcc = new Account(provider, address, wallet)
          return await tmpAcc.execute(calls, ...rest)
        } catch (accErr) {
          console.warn('Account.execute failed, trying wallet.request:', accErr.message)
          // Fallback: wallet direct request
