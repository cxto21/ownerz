...
export async function isStrk20Capable(wallet) {
  try {
    // Create a temporary provider to test capability
    const provider = new RpcProvider({ nodeUrl: RPC_URL })
    const tempAccount = await WalletAccountV6.connect({ nodeUrl: RPC_URL }, wallet)
    
    // Check supported Wallet API version
    const supported = await walletV6.supportedWalletApi(wallet)
    const version = supported?.walletApi?.version || '0.0.0'
    
    // Compare versions
    const [major, minor, patch] = version.split('.').map(Number)
    const [minMajor, minMinor, minPatch] = STRK20_MIN_API_VERSION.split('.').map(Number)
