...
// RPC provider URL from centralized network config
...
export async function waitForWallets(maxAttempts = 5, intervalMs = 500) {
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    if (attempt >= 1) {
      console.log(
        `No wallets found (attempt ${attempt + 1}/${maxAttempts}), retrying in ${delay}ms...`
      );
    }
...
export async function resolvePrivacyWallet(preferredAddress) {
  try {
    ...
    const { capable: privacyCapable, versions: walletApiVersions } =
      await detectStrk20Capability(selected);
    console.log(
      'STRK20 capable:',
      privacyCapable,
      'versions:',
 ...
 */
export async function strk20InvokeViaWalletApi(
  account,
  actions,
  timeoutMs = 45000
) {
  ...
  const walletPromise =
    walletApi && typeof walletApi.request === 'function'
      ? walletApi.request({
  ...
  try {
    ...
    if (!result || !result.transaction_hash) {
      console.warn(
        'STRK20 wallet returned no transaction_hash — user likely rejected'
      );
    ...
    if (err.message === 'WALLET_TIMEOUT') {
      console.warn(
        'STRK20 wallet response timeout — tx was likely submitted on-chain'
      );
      return {
