...
export async function connectWallet(wallet) {
  try {
    ...
    try {
      ...
      // Fallback: wallets that don't expose standard:connect (some Ready versions)
      // Use the older walletV6.requestAccounts pattern
      console.warn('WalletAccountV6.connect failed, trying legacy fallback:', v6Err.message)
      try {
        const accounts = await walletV6.requestAccounts(wallet)
        const addr = Array.isArray(accounts) ? accounts[0] : accounts
        // Build a minimal account-like object from the wallet
        account = {
        ...
        // Final fallback: Account from starknet.js with wallet as signer
        console.warn('Legacy fallback also failed, using Account wrapper:', legacyErr.message)
        const accounts = await wallet.request({ type: 'wallet_requestAccounts' })
        const addr = Array.isArray(accounts) ? accounts[0] : accounts
        account = new (await import('starknet')).Account(provider, addr, wallet)
      }
    }
    const address = account.address || account.accountAddress
