...
export async function isStrk20Capable(wallet) {
  try {
    ...
    console.log('Checking wallet for STRK20:', {
      name: wallet.name,
      id: wallet.id,
      features: wallet.features,
    })
    
    // features might be an object or array
    const features = wallet.features || {}
    const featureKeys = Array.isArray(features) ? features : Object.keys(features)
    
    console.log('Feature keys:', featureKeys)
    
    // Check for STRK20-related features
    const hasStrk20 = featureKeys.some(f => 
      f.includes('strk20') || f.includes('privacy') || f.includes('walletApi')
    )
    
    if (hasStrk20) {
      console.log('STRK20 detected via features')
      return true
    }
    
    // Check wallet properties
    if (wallet.strk20 || wallet.privacy) {
      console.log('STRK20 detected via wallet properties')
      return true
    }
    
    // Ready and Xverse support STRK20
    const walletName = (wallet.name || '').toLowerCase()
    const walletId = (wallet.id || '').toLowerCase()
    
    if (walletName.includes('ready') || walletId.includes('ready')) {
      console.log('STRK20 detected via wallet name (Ready)')
      return true
    }
    
    if (walletName.includes('xverse') || walletId.includes('xverse')) {
      console.log('STRK20 detected via wallet name (Xverse)')
      return true
    }
    
    console.log('STRK20 not detected')
    return false
  } catch (error) {
