...
function fromUserAgent(ua) {
  let m;
  if ((m = ua.match(/(?:Edg|Chromium|Chrome)\/(\d+)/)))
    return { brand: 'chromium', major: parseInt(m[1], 10) };
  if ((m = ua.match(/Firefox\/(\d+)/)) || (m = ua.match(/rv:(\d+)/)))
    return { brand: 'firefox', major: parseInt(m[1], 10) };
  if ((m = ua.match(/Version\/(\d+)(?:\.\d+)*\s+(?:Mobile\/\w+\s+)?Safari/)))
    return { brand: 'safari', major: parseInt(m[1], 10) };
