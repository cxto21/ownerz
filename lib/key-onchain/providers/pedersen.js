...
/**
 * Encode a short string (≤31 bytes) as felt252 (Cairo short string).
 * Each ASCII char becomes one byte, big-endian.
 */
...
/**
 * Decode felt252 back to a short string (Cairo short string).
 */
export function feltToString(felt) {
  if (!felt || felt === '0x0' || felt === 0) return ''
  let n = typeof felt === 'bigint' ? felt : BigInt(felt)
  if (n === 0n) return ''
  const bytes = []
  while (n > 0n) {
