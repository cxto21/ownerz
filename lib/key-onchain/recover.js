...
export async function recoverListing({ cid, claimSecret, account }) {
  // Normalize inputs
  const cleanSecret = String(claimSecret || '')
    .trim()
    .toLowerCase();
  const cleanCid = String(cid || '').trim();
  if (!cleanSecret || cleanSecret.length < 4)
    throw new Error('Invalid claim secret — must be 32-char hex from seller');
  ...
  if (!keySeedCiphertext)
    throw new Error('Key seed not found in storage for CID ' + cleanCid);
  ...
  const toBigIntHex = (v) => {
    try {
      return BigInt(v).toString(16).toLowerCase();
    } catch {
  ...
  const matches = storedCandidates.some((s) => {
    ...
    return (
      sl === computedHex ||
      String(s).toLowerCase() === String(keySeedCiphertext).toLowerCase() ||
      sl === rawHex
    );
  });
  if (!matches) {
    ...
    console.warn(
      '[recover] integrity mismatch — stored',
      stored0,
      'computed',
      integrityHash,
      'rawLen',
      String(keySeedCiphertext).length,
      'isOldVault',
      isOldVault,
      'locked',
      JSON.stringify(locked, (_, v) =>
    ...
    if (!isOldVault) {
      throw new Error(
        `Integrity check failed — data may be tampered (stored ${stored0} vs computed ${integrityHash.slice(0, 18)}...)`
      );
    } else {
      console.warn(
        '[recover] old vault — skipping strict integrity check, proceeding to unwrap'
      );
    }
  } else {
    console.log(
      '[recover] integrity verified',
      integrityHash.slice(0, 18) + '...'
    );
  }
  ...
  try {
    ...
    if (
      ...
      String(e.message).includes('Operation')
    ) {
      throw new Error(
        'Invalid claim secret — unwrap failed. Check the secret from seller (32 hex chars, lowercase).'
      );
    }
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        'Invalid claim secret — INVALID_PROOF. On-chain commitment mismatch.'
      );
    }
    throw e;
  }
  ...
  try {
    ...
    throw new Error(
      'Decryption failed — file may be corrupted or key mismatch: ' +
        (e.message || String(e))
    );
  }
