...
export function createKeyExchangeMockupProvider({ contractAddress, provider }) {
   ...
   * @param {string} params.identifier - vault_uid hashed to felt252 (0x...)
   * @param {string} params.commitment - felt252
   * @param {string} params.integrityHash - felt252
   * @param {Object} params.meta - { price: bigint, ttl: number, fee: bigint, pqc: bool, tokenGate: string, fileCid: string }
   */
  async function lock({ account, identifier, commitment, integrityHash, meta }) {
    ...
    const tokenGate = meta?.tokenGate || '0x0'
    const fileCid = meta?.fileCid || '0x0'
    ...
    // FileVault.create_vault(cid, price, integrity_hash, commitment, ttl, pqc, token_gate, file_cid)
    // cid = hash(vault_uid) as identifier, file_cid = actual CID for file retrieval
  ...
  async function unlock({ account, identifier, proof }) {
    ...
    // Use account.execute directly — more reliable than contract.claim_vault
    // for u16 encoding across different wallet implementations
    const result = await account.execute({
      contractAddress: fileVaultAddress,
      entrypoint: 'claim_vault',
      calldata: [identifier, proof],
    })
    return result
  }
  ...
  async function readLock(identifier) {
    ...
    try {
      ...
      let feeBpsVal = vault.platform_fee_bps ?? vault.platformFeeBps ?? vault.platform_feeBps ?? 100
