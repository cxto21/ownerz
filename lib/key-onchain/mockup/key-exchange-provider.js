...
const { rpcFallback: DEFAULT_RPC } = getNetworkConfig();
...
export function createKeyExchangeMockupProvider({ contractAddress, provider }) {
  ...
  async function getOnChainAbi() {
    ...
    try {
      ...
      console.warn(
        '[readLock] getClassAt failed, using local ABI',
   ...
   * @param {Object} params.meta - { price: bigint, ttl: number, fee: bigint, pqc: bool }
   */
  async function lock({
    account,
    identifier,
    ...
    if (!fileVaultAddress)
      throw new Error(
        'FileVault address not set (NEXT_PUBLIC_FILEVAULT_ADDRESS)'
      );
    ...
    // FileVault.create_vault(cid, price, integrity_hash, commitment, ttl, pqc)
    // pqc as felt 1/0, platform_fee_bps hardcoded to 100 inside contract
    ...
    try {
      ...
      if (Array.isArray(result)) {
        ...
        if (!vault || !lock) {
          ...
          const ih =
            vault.integrity_hash ??
            vault.integrityHash ??
            vault.key_seed_ciphertext ??
            '0x0';
          lock = {
      ...
      if (!vault || vault.seller === undefined) {
        ...
        console.warn(
          '[readLock] empty vault for identifier',
          identifier,
          'raw:',
          result
        );
      ...
      try {
        if (vault.price !== undefined && vault.price !== null) {
          if (
            typeof vault.price === 'object' &&
            vault.price.low !== undefined &&
            vault.price.high !== undefined
          ) {
            ...
            typeof vault.price === 'string' ||
            typeof vault.price === 'number'
          ) {
        ...
        console.warn(
          '[readLock] price parse failed, using 0:',
          vault.price,
          priceErr.message
        );
        priceBig = BigInt(0);
      ...
      // Normalize pqc and platform_fee_bps (new Vault fields, graceful fallback for old vaults)
      let pqcVal = vault.pqc ?? vault.PQC ?? false;
      // starknet.js may return true/false or 1/0 or BigInt
      if (typeof pqcVal === 'bigint') pqcVal = pqcVal !== BigInt(0);
      else if (typeof pqcVal === 'number') pqcVal = pqcVal !== 0;
      else if (typeof pqcVal === 'string')
        pqcVal = pqcVal === '1' || pqcVal === 'true';
      const pqcBool = Boolean(pqcVal);
      let feeBpsVal =
        vault.platform_fee_bps ??
        vault.platformFeeBps ??
        vault.platform_feeBps ??
        100;
      const feeBpsNum = Number(feeBpsVal) || 100;
      ...
      const normalized = {
        ...
        pqc: pqcBool,
        platformFeeBps: feeBpsNum,
        platform_fee_bps: feeBpsNum,
