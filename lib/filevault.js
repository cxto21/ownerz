...
if (typeof window !== 'undefined') {
  console.log(
    '[filevault] ABI loaded:',
    Array.isArray(FileVaultABI),
    FileVaultABI?.length,
    'entries'
  );
}
...
const KEX_ADDRESS = process.env.NEXT_PUBLIC_KEY_EXCHANGE_MOCKUP_ADDRESS;
...
export async function createVault(
  ...
  if (!ih)
    throw new Error('integrityHash required (or legacy keySeedCiphertext)');
  ...
  console.log(
    '[createVault] multicall: approve + create_vault, fee:',
    feeAmount.toString(),
    'integrityHash:',
    ih
  );
...
export async function claimVault(account, cid, claimSecret) {
  ...
  try {
    ...
    if (
      e.message?.includes('INVALID_PROOF') ||
      e.message?.includes('0x494e56414c49445f50524f4f46')
    ) {
...
export async function getVault(provider, cid) {
  ...
  try {
    ...
    if (
      ...
      priceBigInt =
        BigInt(vault.price.low) + (BigInt(vault.price.high) << BigInt(128));
    } else if (
      typeof vault.price === 'string' ||
      typeof vault.price === 'bigint'
    ) {
    ...
    const feeBps =
      Number(vault.platform_fee_bps ?? vault.platformFeeBps ?? 100) || 100;
...
export async function deployContract(account) {
  ...
  throw new Error(
    'Use scripts/deploy.js for FileVault v2 (requires KeyExchangeMockup address). See .env.example'
  );
}
