 ...
 * FileVault v2 signature: create_vault(cid, price:u256, integrity_hash:felt, commitment:felt, ttl:u64, pqc:felt, token_gate:ContractAddress, file_cid:felt)
 ...
 * @param {string} params.cid - Content identifier (felt252) — identifier = hash(vault_uid)
 ...
 * @param {string} params.tokenGate - Optional token gate address (0x0 = public)
 * @param {string} params.fileCid - Optional file_cid (0x0 = invite without file)
 * @returns {Promise<Object>} Transaction result
 */
export async function createVault(account, { cid, price, integrityHash, integrity_hash, commitment, keySeedCiphertext, ttl, fee, pqc, tokenGate, token_gate, fileCid, file_cid }) {
  if (!VAULT_ADDRESS) throw new Error('NEXT_PUBLIC_FILEVAULT_ADDRESS not set')
  ...
  const resolvedTokenGate = tokenGate || token_gate || '0x0'
  const resolvedFileCid = fileCid || file_cid || '0x0'

  // Multicall: approve STRK to vault contract, then create_vault (new v2 includes token_gate + file_cid)
  const createVaultCalldata = [
    cid,
    priceLow.toString(),
    priceHigh.toString(),
    ih,
