...
export default async function handler(req) {
  ...
  try {
    ...
    const fileName =
      body.fileName || body.filename || body.originalName || 'unnamed.enc';
    const sellerAddress = body.sellerAddress || '';
    const price = body.price || '0';

    if (!encryptedData) {
      return new Response(
        JSON.stringify({ error: 'Missing encryptedData or data' }),
        { status: 400 }
      );
    }
    ...
    const tlsVersion =
      req.cf?.tlsVersion || req.headers.get('cf-tls-version') || '';
